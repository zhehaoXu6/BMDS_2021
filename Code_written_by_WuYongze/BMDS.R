...
#计算后验联合概率密度函数
log_P = function(Y, X, sigma2, lambda, a0, b0, alpha0, beta0, p) {
  #sigma2的IG先验的参数
  a = a0
  b = b0
  #lambda的IG先验的参数
  ...
  p_12 = sum((-(n / 2)) * log(lambda))#p的第一部分
  p_13 = sum((-(alpha + 1)) * log(lambda))
  p_1 = p_11 + p_12 + p_13
  D_ij = deta(Y, dim(Y)[2])
  deta_ij = deta(X, p)
  ...
  for (i in seq(1:rows_Xnew)) {
    for (j in seq(1:cols_Xnew)) {
      X_new[i, j] = X_new[i, j] / lambda[j]
      p_23 = p_23 + X_new[i, j]
    }
  }
  p_23 = -p_23 / 2
  p_24 = -b / sigma2
  p_25 = -sum(beta / lambda)
...
ALSCAL = function(Y, p, niter) {
  ...
  lambda_chain = array(0, dim = c(p, niter))#lambda的链 p行niter列
  ...
  for (iter in 1:niter) {
    lambda_a = alpha0 + n / 2
    lambda_b = array(0, dim = c(p))
    for (j in 1:p) {
      beta0[j] = var(X[, j])
      lambda_b[j] = beta0[j] + var(X[, j]) * n / 2
      lambda[j] = 1 / rgamma(1, lambda_a, lambda_b[j])
    }
    lambda_chain[, iter] = lambda
    
    X_new =  array(1, dim = c(n, p))
    for (i in 1:n) {
      X_new[i, ] = mvrnorm(n = 1, rep(X[i, ]), Xi_sigma2)
    }
    log_X = log_P(Y, X_new, sigma2, lambda, a0, b0, alpha0, beta0, p) - log_P(Y, X, sigma2, lambda, a0, b0, alpha0, beta0, p)
    ...
    log_sigma2 = log_P(Y, X, sigma2_new, lambda, a0, b0, alpha0, beta0, p) -
      log_P(Y, X, sigma2, lambda, a0, b0, alpha0, beta0, p)
...
MDSIC = function(Y, niter, p) {
  ...
  if (p == 1) {
    ...
    for (i in 1:(p - 1)) {
      #   SSR1 = ALSCAL(Y,i,niter)[[8]]
      #   print(SSR1)
      #   SSR2 = ALSCAL(Y,i+1,niter)[[8]]
      #   print(SSR2)
      #   print(i)
      print(i)
      X1_chain = ALSCAL(Y, i, niter)[[1]]
      X1 = X1_chain[, , niter]
      deta1 = deta(X1, i)
      SSR1 = SSR_old(D, deta1)
      #   print(X1)
      X2_chain = ALSCAL(Y, i + 1, niter)[[1]]
      X2 = X2_chain[, , niter]
      deta2 = deta(X2, i + 1)
...
#算法使用部分
Y = read.csv("./Data/600519.csv", header = T)
niter = 1000
p = 1
out = ALSCAL(Y, p, niter)
X_chain = out[[1]]
sigma2_chain = out[[2]]
lambda_chain = out[[3]]
...
print(MDSICp)
#绘制图
plot(X_chain[2, 1, ], type = 'l',xlab='Iteration')
plot(sigma2_chain,type = 'l',xlab='Iteration')
plot(lambda_chain[1,],type = 'l',xlab='Iteration')
#rate_X = rate_X/niter
